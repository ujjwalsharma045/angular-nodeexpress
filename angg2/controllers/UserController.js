module.exports = function(app , func , mail, upload, storage, mailer, multer, validator, User, paginate , cors , dateFormat , dateDiff, dobByAge, json2csv, excel , pdf, passport , LocalStrategy, bCrypt , fs, async, PasswordGenerate, randtoken, handlebars, UserProfile , path){ 
	...
	app.get("/showusers", function(req, res){
            ...
            if(req.query.searchcontent){
				data = {
					$or:[
					   {'first_name':{"$regex":req.query.searchcontent , "$options": "i"}},
					   {'last_name':{"$regex":req.query.searchcontent , "$options": "i"}},
					   {'email':{"$regex":req.query.searchcontent , "$options": "i"}},
					   {'username':{"$regex":req.query.searchcontent , "$options": "i"}},
					   {'address':{"$regex":req.query.searchcontent , "$options": "i"}},
					   {'city':{"$regex":req.query.searchcontent , "$options": "i"}},
					   {'zipcode':{"$regex":req.query.searchcontent , "$options": "i"}}
					]
				};
			}	
	...
	app.get("/view/:id", function(req, res){
		...
		User.find({_id:userid}, function(err, records) {
			  if (err) throw err;
			  console.log(records); 
			  var filepath = "uploads/noimage.jpg";
			  if(records[0].profile_pic!=""){
				  if(fs.existsSync("uploads/"+records[0].profile_pic)){
					  filepath = "uploads/"+records[0].profile_pic
				  }				  
			  }
			  
			  res.setHeader('Content-Type', 'application/json');
			  res.send(JSON.stringify({'records':records, 'success':1, 'authen':1 , 'filepath':filepath}));
		}); 		
	});
